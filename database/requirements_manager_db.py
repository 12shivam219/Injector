        ...
        try:
            with self._get_session_with_context(user_id=user_id) as session:
                ...
                requirement = Requirement(
                    ...
                    nice_to_have_skills=requirement_data.get('nice_to_have_skills', []),
                    job_location=requirement_data.get('job_location', ''),
                    remote_option=requirement_data.get('remote_option', False),
                    salary_range_min=requirement_data.get('salary_range_min'),
                    salary_range_max=requirement_data.get('salary_range_max'),
                    currency=requirement_data.get('currency', 'USD'),
                    application_deadline=requirement_data.get('application_deadline'),
                    application_status=requirement_data.get('application_status', 'Not Applied'),
                    priority_score=requirement_data.get('priority_score', 5),
                    source_url=requirement_data.get('source_url'),
                    referral_source=requirement_data.get('referral_source'),
        ...
        try:
            with get_db_session() as session:
                ...
                if filters:
                    ...
                    if 'priority_score' in filters:
                        query = query.filter(Requirement.priority_score >= filters['priority_score'])
                    
                    if 'remote_option' in filters:
                        query = query.filter(Requirement.remote_option == filters['remote_option'])
                    
                    if 'application_status' in filters:
                        query = query.filter(Requirement.application_status == filters['application_status'])
                    
                    if 'salary_range' in filters:
                        min_salary, max_salary = filters['salary_range']
                        if min_salary:
                            query = query.filter(Requirement.salary_range_min >= min_salary * 100)  # Convert to cents
                        if max_salary:
                            query = query.filter(Requirement.salary_range_max <= max_salary * 100)
                    
                    if 'tags' in filters:
                        for tag in filters['tags']:
                            query = query.filter(Requirement.tags.contains([tag]))
