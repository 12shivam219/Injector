...
class FormatAnalyzer:
    ...
    def _extract_name_pattern(self, doc: Document) -> Dict[str, str]:
        ...
        try:
            ...
            for i, para in enumerate(doc.paragraphs[:5]):
                if para.text.strip() and any(run.bold or run.font.size or run.font.size > 12 for run in para.runs):
    ...
    def _extract_company_patterns(self, doc: Document) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for i, para in enumerate(doc.paragraphs):
                ...
                if in_experience:
                    # Look for company entries (often bold, followed by dates)
                    if any(run.bold for run in para.runs) and re.search(self.date_pattern, text):
                        current_company = {
                            'pattern': text,
    ...
    def _match_sections(self, new_patterns: Dict, stored_format: ResumeFormat) -> float:
        ...
        try:
            if not new_patterns.get('section_patterns') or not stored_format.section_patterns:
                return 0
